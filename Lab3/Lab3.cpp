...
void DDA(int x1, int x2, int y1, int y2, COLORREF color)
{
    float i, l;
    float x, y, dx, dy;
...
void DDApoint(vector<point>& shape1, COLORREF color)
{
    float i, l;
    float x, y, dx, dy;
...
void brezenham(int x1, int x2, int y1, int y2, COLORREF color)
{
...
void areaA(int xz, int yz, COLORREF color)
{
    ...
    while (!stack.empty())
    {
        ...
        //auto it = find_if(stA.begin(), stA.end(), [&](const point& s)->bool { return (s.x == p.x && s.y == p.y); });
        /*if (auto it = find_if(stA.begin(), stA.end(), [&](const point& s)->bool { return (s.x == p.x && s.y == p.y); }); it != end(stA))
        {

        }  */  //если такого элемента еще нет, то добавляем его
        //stA.push_back(p);
        metricsA++;
        int x = p.x;
        int y = p.y;
...
int main()
{
    ...
    if (test == 0)
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            for (int j = 0; j < shapeAll[i].size() - 1; j++)
            {
                brezenham(shapeAll[i][j].x, shapeAll[i][j + 1].x, shapeAll[i][j].y, shapeAll[i][j + 1].y, colorAll[i]);
            }
            brezenham(shapeAll[i][shapeAll[i].size() - 1].x, shapeAll[i][0].x, shapeAll[i][shapeAll[i].size() - 1].y, shapeAll[i][0].y, colorAll[i]);
    ...
    if(test == 1)
    {
        for (int i = 0; i < 8; i++)
        {
            for (int j = 0; j < shapeAll[i].size() - 1; j++)
            {
                DDA(shapeAll[i][j].x, shapeAll[i][j + 1].x, shapeAll[i][j].y, shapeAll[i][j + 1].y, colorAll[i]);
            }
            DDA(shapeAll[i][shapeAll[i].size() - 1].x, shapeAll[i][0].x, shapeAll[i][shapeAll[i].size() - 1].y, shapeAll[i][0].y, colorAll[i]);
            ...
            for (int j = 0; j < shapeAll[i].size() - 1; j++)
            {
                brezenham(shapeAll[i][j].x, shapeAll[i][j + 1].x, shapeAll[i][j].y, shapeAll[i][j + 1].y, 0x00FF0000);
            }
            brezenham(shapeAll[i][shapeAll[i].size() - 1].x, shapeAll[i][0].x, shapeAll[i][shapeAll[i].size() - 1].y, shapeAll[i][0].y, 0x00FF0000);
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            start_time = clock(); // начальное время
            DDA(100, round(x2), 200, round(y2), 0x000000FF);
            end_time = clock(); // конечное время
            search_time = end_time - start_time; // искомое время
            cout << "DDA:" << search_time << " ";
            metricsDDA = 0;
            start_time = clock(); // начальное время
            brezenham(100, round(x2), 200, round(y2), 0x0000FF00);
    ...
    if (test == 2)
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            DDApoint(shapeAll[i], colorAll[i]);
            areaB(zPointAll[i].x, zPointAll[i].y, colorAll[i]);
        }
    }
